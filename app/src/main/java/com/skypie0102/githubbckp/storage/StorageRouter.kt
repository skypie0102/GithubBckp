...
class StorageRouter @Inject constructor(
    ...
    private val googleDrive: Provider<GoogleDriveStorageProvider>,
    private val documentTree: Provider<DocumentTreeStorageProvider>,
) : StorageProvider {
    ...
    private fun provider(destination: StorageDestination): StorageProvider = when (destination) {
        StorageDestination.GOOGLE_DRIVE -> googleDrive.get()
        StorageDestination.DOCUMENT_TREE -> documentTree.get()
    }
}
