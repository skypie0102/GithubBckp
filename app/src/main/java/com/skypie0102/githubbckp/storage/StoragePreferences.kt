...
class StoragePreferences @Inject constructor(
    ...
    fun isConfigured(): Boolean = documentTreeUri() != null

    fun persistDocumentTree(uri: Uri) {
        val flags = Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION
        context.contentResolver.takePersistableUriPermission(uri, flags)
        preferences.edit().putString(KEY_DOCUMENT_TREE_URI, uri.toString()).apply()
    }

    fun clear() {
        preferences.edit().remove(KEY_DOCUMENT_TREE_URI).apply()
    }

    private companion object {
        const val PREFERENCES_NAME = "local-backup-storage"
