...
class GitLfsDownloadService @Inject constructor(
    ...
    /**
     * Read-only proof used after a recovery drill publishes LFS. Every unique
     * pointer must be advertised for download by the target repository. A small
     * deterministic sample is downloaded again and fully checked by size/SHA-256
     * so the drill proves both object availability and actual byte readability.
     */
